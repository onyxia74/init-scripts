...
# Purpose: déploiement d'une application de datavisualisation Streamlit
...
cat > "${WORK_DIR}/app.py" <<'ONYXIA_FILE_27aff9255b00'
import streamlit as st
import pandas as pd
import numpy as np

st.title('Ma Dashboard de Dataviz')
st.write('Bienvenue dans votre application de visualisation.')

data = pd.DataFrame(np.random.randn(20, 3), columns=['A', 'B', 'C'])
st.line_chart(data)
ONYXIA_FILE_27aff9255b00
mkdir -p "$(dirname "${WORK_DIR}/requirements_txt")"
cat > "${WORK_DIR}/requirements_txt" <<'ONYXIA_FILE_931deb4f55a2'
streamlit
pandas
numpy
ONYXIA_FILE_931deb4f55a2
